    ...
    try:
        while True:
            ...
            if image is not None and resolution is not None:
                ...
                if green_stripe_distance is not None and green_stripe_distance <= stop_distance:
                    green_center_vertical = get_green_stripe_vertical_center(img_np)
                    if green_center_vertical is not None and abs(green_center_vertical - resolution[1] / 2) < 10: # Distanza soglia dal centro verticale
                        print("Linea orizzontale davanti al robot, fermando il robottino.")
...
max_correction = 50  # Da regolare in base alle dimensioni dell'immagine e alla logica di controllo
stop_distance = -150
image_width = 640
