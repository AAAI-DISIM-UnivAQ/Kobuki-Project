...
def turn_left():
    ...
    initial_angle = sim.getObjectOrientation(
        robot_handle, -1)[2]  # Angolo iniziale
    ...
    while True:
        current_angle = sim.getObjectOrientation(
            robot_handle, -1)[2]  # Angolo corrente
...
# Handle del robot
# Sostituisci con il percorso corretto del robot nel tuo ambiente
...
try:
    while True:
        ...
        print("sensors:", on_line_left, on_line_front, on_line_right)
        ...
        if on_line_front and (((left_distance is not None and left_distance < intersection_threshold) and on_line_left) or (
            ...
            if options:  # Se ci sono opzioni disponibili
                ...
                if direction == "Right":
                    ...
                    go_straight()
        elif not on_line_front and (((left_distance is not None and left_distance < intersection_threshold) and on_line_left) or (
                (right_distance is not None and right_distance < intersection_threshold) and on_line_right)):
            sim.setJointTargetVelocity(left_wheel_handle, 0)
            sim.setJointTargetVelocity(right_wheel_handle, 0)
            print("Intersection detected, choosing direction...")
